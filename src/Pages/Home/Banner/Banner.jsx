            ...
            >
                {
                    bannerData.map((banner, index) => <SwiperSlide key={index}>
                        <div className='w-full h-[80vh] md:h-screen'>
                            <div style={{backgroundImage: 'url("/banner.jpg")'}} className={`bg-cover bg-center h-full w-full text-white flex gap-5 items-center justify-between px-10 md:pl-24 ${index === 1 && 'flex-row-reverse'}`}>
                                <div className='space-y-5 w-full md:w-1/2'>
                                    <h3 className='text-3xl md:text-5xl font-semibold text-gray-100 ps-3'>{banner.headline}</h3>
                                    <p className='text-base md:text-lg ps-3 text-gray-100 border-l-4'>{banner.subheadline}</p>
